...
@section('content')
    <div class="bg-gradient-to-r from-coffee-dark to-[#005bb5] rounded-2xl p-8 mb-8 text-white shadow-lg relative overflow-hidden">
        <div class="relative z-10 w-full md:w-2/3">
            <h2 class="text-2xl font-bold mb-2">Selamat Datang, {{ auth()->user()->name ?? 'Super Admin' }}! 👋</h2>
    ...
    <div class="grid grid-cols-1 {{ auth()->user()->role === 'superadmin' ? 'md:grid-cols-3' : 'md:grid-cols-2' }} gap-6 mb-8">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between group hover:border-coffee-primary transition duration-300">
            <div>
                ...
                <h3 class="text-3xl font-bold text-coffee-dark">{{ $totalMenu ?? 0 }}</h3>
                <p class="text-[10px] text-green-500 mt-2 font-medium">Menu terdaftar</p>
            </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between group hover:border-[#8A735E] transition duration-300">
            <div>
                ...
                <h3 class="text-3xl font-bold text-coffee-dark">{{ $totalGallery ?? 0 }}</h3>
                <p class="text-[10px] text-green-500 mt-2 font-medium">Foto dipublikasi</p>
            </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between group hover:border-orange-400 transition duration-300">
            <div>
                ...
                <h3 class="text-3xl font-bold text-coffee-dark">{{ $totalAdmin ?? 0 }}</h3>
    ...
    <div class="grid grid-cols-1 {{ auth()->user()->role === 'superadmin' ? 'lg:grid-cols-2' : '' }} gap-8">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-100 transition">
                    <div class="flex items-center">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($admin->name) }}&background=023A73&color=fff" class="w-10 h-10 rounded-full">
                        <div class="ml-3">
                ...
                @empty
                    <p class="text-center text-xs text-gray-500">Belum ada data admin.</p>
                @endforelse
            </div>
        </div>
        @endif
